...
class PSRLStrategy:
    ...
    def run(self, T_0, starting_episode_num, num_episodes, experiment_num,
        ...
        while t <= 2 * T_max:       # it is multiplied by 2 since each sample is a couple
            ...
            self.psrl_policy.update_policy_infos(
                state_action_transition_matrix=optimistic_transition_matrix_mdp,
            ...
            if self.collected_samples is None:
                ...
                last_samples = self.collected_samples[-num_last_samples_for_belief_update:]

            self.psrl_policy.update_belief_from_samples(
                action_obs_samples=last_samples
            )


            while t <= t_k + T_k_1 and t <= 2 * T_max and np.all(m_t <= 2 * m_tk):

                first_action = self.psrl_policy.choose_action()
                ...
                self.psrl_policy.update(first_action, first_obs)
                ...
                if first_sample is True:
                    if len(episode_collected_samples) > 0:
                        first_sample = False
    ...
    def update_dirichlet_prior(self, last_samples: list):
        ...
        first_action, first_obs = last_samples[0][0], last_samples[0][1]
        second_action, second_obs = last_samples[1][0], last_samples[1][1]

        first_belief = self.psrl_policy.previous_discretized_belief
        ...
        final_update = final_update / final_update.sum() / 5
    ...
    def sample_transition_matrix(self):
        ...
        for state in range(self.num_states):
            for action in range(self.num_actions):
                ...
                sampled_dirichlet = np.random.dirichlet(current_dirichlet_prior)
                arr_clipped = np.clip(sampled_dirichlet, 0.0001, None)
                self.estimated_transition_matrix[state, action] = arr_clipped
    ...
    def save_results(self,
        ...
        if isinstance(self.psrl_policy.discretized_belief_index, int):
            index_to_store = self.psrl_policy.discretized_belief_index
        else:
            index_to_store = self.psrl_policy.discretized_belief_index.tolist()

        result_dict = {
            "starting_state": starting_state.tolist(),
            "discretized_belief": self.psrl_policy.discretized_belief.tolist(),
        ...
        basic_info_path = f"/{self.epsilon_state}stst_{self.min_action_prob}_minac_{self.save_path_num}/{T_0}_init"
        dir_to_create_path = self.save_path + basic_info_path
        if not os.path.exists(dir_to_create_path):
            os.mkdir(dir_to_create_path)
        f = open(
    ...
    def init_policy(self):
        ...
        # used policy
        self.psrl_policy = PSRLDiscretizedBeliefBasedPolicy(
            num_states=self.num_states,
            num_actions=self.num_actions,
            num_obs=self.num_obs,
